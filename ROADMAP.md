...
- [x] **4.9 Recovery & Strain circular progress rings** — **Implemented (2026-07-14).** Reusable `ProgressRing` (`src/components/charts/ProgressRing.tsx`, barrel-exported): plain SVG `<circle>` arc via `stroke-dasharray`/`stroke-dashoffset` (`offset = circumference × (1 − fraction)`, fraction clamped, non-finite → 0) — no `d3-shape`, per this task's note. §5.2-compliant: `role="img"` + `<title>`/`<desc>` via `aria-labelledby` (ChartSvg pattern; the desc carries value + zone + scored day — rule 2's fallback for a single scalar), centered value is real SVG text in `--color-text` (never zone-colored, §5.1), entrance fill gated on `prefers-reduced-motion` in JS (`chartTransitionDuration`, StackedBarChart pattern) AND CSS (`charts.css` kills the transition). `noData` renders bare track + muted "—" + "no data yet" caption with an honest desc (mirrors the old placeholder's label). **Zone cutoffs VERIFIED against https://developer.whoop.com/docs/whoop-101/ (fetched 2026-07-14): green 67–100%, yellow 34–66%, red 0–33%** — constants live in `RECOVERY_ZONES` (`src/App.tsx`) with the citation; same doc confirms strain's 0–21 Borg scale (`STRAIN_SCALE_MAX`). Zone hues = fill-safe UI tokens (`--color-positive`/`--color-warning`/`--color-negative`); strain ring = `--color-chart-5` per §4. Wiring: ONE `useDailySeries(7)` call in `App` feeds both `RecoveryRingTile`/`StrainRingTile` (per-tile hooks would double-fetch identical rows); each tile scans the series from the end for its own latest non-null day, so a PENDING_SCORE/UNSCORABLE today falls through to yesterday (or to `noData`) via the standing null discipline. Fetch state → ChartContainer: loading/error pass through, 401 → `empty` with a connect-WHOOP message, ready-but-unscored → the ring's `noData` (NOT `empty`). Dead `.stat-donut*` CSS/markup removed. Lint + `tsc -b` + `typecheck:api` + prettier pass; visually verified in the dev preview via the temporary vite `/api/daily-series` mock (green 72%, red 28%, strain 6.3/21, strain-noData; `aria-labelledby` resolution confirmed in-browser), then the mock was fully reverted.
- [ ] **4.10 Period meter — dot-matrix cycle-day progress bar** — replace the static period-meter placeholder (bento `period` tile) with a real dot-matrix row showing which day of the cycle the user is on. **Data source confirmed (2026-07-14): self-reported, not WHOOP API.** Verified directly against the live WHOOP v2 OpenAPI spec (`https://api.prod.whoop.com/developer/doc/openapi.json`) — the full resource list is Activity ID Mapping, Partner, User, Cycle, Recovery, Sleep, Workout; nothing exposes menstrual cycle data, so this was never fetchable from WHOOP.
  - **Entry point confirmed (2026-07-14, revised): the Phase 5 daily journal, not a standalone input.** The journal stub already lists "Period" as one of its planned fields (`src/App.tsx`'s `journal-stub-list` — Hydrated, Cramps, Period, Discharge, Afternoon snack, Traveled, Caffeine, Alcohol). **This makes 4.10 dependent on Phase 5 shipping the journal (specifically the "Period" field) before the meter can show real data** — correcting the earlier plan here, which assumed a standalone input to avoid that dependency.
  - **What 4.10 itself builds now:** the dot-matrix rendering component and the day-of-cycle computation logic, built against the eventual journal-logged period-start dates. `today − most_recent_period_start_date + 1`, cycle length estimated from historical logged starts once ≥2 exist, else a flagged assumed default (never presented as fact). Same empty-state discipline as the rest of Phase 4 (§5.1): honest "no data yet" until Phase 5 exists and the user has actually logged at least one period start — never a fabricated or default-day-1 display.
  - **Sequencing consequence:** the period-meter tile stays in its placeholder/empty state through the rest of Phase 4; it only goes live once Phase 5 (specifically the journal's "Period" field + its storage) ships. Worth deciding whether that's acceptable or whether Phase 5's journal should be pulled forward for this one field — flag if you want to reconsider.
  - **Cycle-start-detection rule — PROPOSAL, needs your confirmation (2026-07-14):** the journal only logs "Period: yes" per day, one day at a time — it doesn't ask the user to mark "this is day 1." So day 3 of an ongoing period and day 1 of a brand-new period look identical in the raw data (both just a "yes" for that date); the app has to infer which is which. Proposed algorithm:
    1. Pull every day where the journal's Period field is explicitly `yes` (see the schema requirement below — this only works if "not logged" is distinguishable from "logged no"), sorted chronologically.
    2. Group them into **episodes**: a `yes` day starts a **new** episode if the gap since the previous `yes` day is **greater than a threshold** (proposed default: **3 days**) — otherwise it's a continuation of the current episode. 3 days is chosen to tolerate a missed logging day or two mid-period without misreading it as a new cycle, while staying far below any realistic full cycle length (even short cycles run ~21+ days), so it shouldn't false-positive on a real new period. **This threshold is a proposed heuristic, not derived from a clinical source — flag if you want a different value, or if you'd rather this be a user-adjustable setting.**
    3. Each episode's **cycle start date** = its first `yes` day. Day-of-cycle shown on the meter = `today − most_recent_episode's start date + 1`, and keeps counting past the period's end (a "cycle" is longer than the bleeding days) until the next episode starts.
    4. **Retroactive edits:** if the user backfills or edits a past day's Period field, episodes must be recomputed from the full history, not just appended to — a late edit could merge, split, or shift episode boundaries.
    5. **First-ever entry:** trivially starts episode 1; no history to compare against.
    6. **Known limitation, disclose in the UI:** this is inferred from daily checkboxes, not an explicit "day 1" action, so edge cases (e.g., spotting with a >3-day gap inside what's actually the same period) can be misread as a new cycle. Consider an optional manual "mark as new cycle start" override in the journal as a Phase 5+ enhancement if this proves unreliable in practice — not required to ship 4.10/5.x, but worth flagging now rather than discovering it later.
  - **Schema requirement this creates for Phase 5.1 (flagging now, applies there):** the "Period" journal field must be **tri-state** — `yes` / `no` / `not logged` — not a plain boolean, because the algorithm above only trusts explicit `yes` entries and must NOT treat a day the user simply didn't open the journal for as "no period" (that would incorrectly break episodes on ordinary missed-logging days). Phase 5.1 needs to decide the field's storage representation (e.g., nullable boolean, or an explicit enum) with this in mind.
...
- [ ] **5.1 Define questions** — e.g. mood, soreness, stress, alcohol, nutrition, motivation (Likert 1–5 or tags). Decide the schema. **Constraint from Phase 4.10 (added 2026-07-14):** the "Period" field specifically must be tri-state (`yes` / `no` / `not logged`), not a plain boolean — the period-meter's cycle-start-detection algorithm needs to distinguish "logged no period today" from "journal wasn't filled in that day," and a plain boolean can't represent that distinction. See ROADMAP.md 4.10 for why.
